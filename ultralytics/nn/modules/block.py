...
from .conv import Conv, DWConv, GhostConv, LightConv, RepConv, autopad, LDD, DSConv, ConvBNAct, EnhancedDepthwiseConv, SimConv
...
class ESSamp(nn.Module):
    def __init__(self, c1, c2, k=3, s=1, act=True, depth_multiplier=2):
        ...
        self.dsconv = EnhancedDepthwiseConv(c1 * 4, c2, k=k, s=s, act=act,depth_multiplier=depth_multiplier)
...
class BasicBlock_3x3_Reverse(nn.Module):
    def __init__(self, ch_in, ch_hidden_ratio, ch_out, shortcut=True):
        ...
        self.conv1 = ConvBNAct(ch_hidden, ch_out, 3, stride=1)
...
class CSPStage(nn.Module):
    def __init__(self, ch_in, ch_out, n, block_fn='BasicBlock_3x3_Reverse', ch_hidden_ratio=1.0, spp=False):
        ...
        self.conv1 = ConvBNAct(ch_in, ch_first, 1)
        self.conv2 = ConvBNAct(ch_in, ch_mid, 1)
        ...
        for i in range(n):
            ...
            next_ch_in = ch_mid
        self.conv3 = ConvBNAct(ch_mid * n + ch_first, ch_out, 1)
...
class DEB(nn.Module):
    """Faster Implementation of CSP Bottleneck with 2 convolutions using LDD blocks."""

    def __init__(self, in_channels, out_channels, kernel_size=3, n=1, e= 0.5, g= 1):
        """Initialize LEDM module.
        ...
        self.s1 = Conv( 2*self.c, self.c, 1)
        self.s2 = Conv( 2*self.c, self.c, 1)
        self.cv2 = Conv((2 + n) * self.c, out_channels, 1)  # optional act=FReLU(c2)
        # Create pairs of LDD modules for element-wise multiplication
        self.m = nn.ModuleList(
            nn.ModuleList([
                LDD(self.c, self.c, kernel_size, 1),
                LDD(self.c, self.c, kernel_size, 1)
            ]) for _ in range(n)
        )

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Forward pass through LDDC3K2 layer."""
        y = self.cv1(x)
        y0,y1 = self.s1(y),self.s2(y)
        y = [y0,y1]
        # Process through pairs of LDD modules and perform element-wise multiplication
        for ldd_pair in self.m:
            ldd1_out = ldd_pair[0](y[-1])
            ldd2_out = ldd_pair[1](y[-1])
            # Element-wise multiplication of outputs from two LDD modules
            multiplied = ldd1_out + ldd2_out
            y.append(multiplied)
        return self.cv2(torch.cat(y, 1))
